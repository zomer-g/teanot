...
3. **בחירה ומיקוד** – הכלי `ask_user` מציג כרטיסים/צ'יפים (נאשם, טווח כמות) ומסיים את התור; התשובה חוזרת כ-`tool_result`. שאלה עם `id: "mode"` מוצגת כטופס הגדרת חיפוש: סימון גזרי דין ו/או הנחיות; לגזרי דין — סדר החומרה (ברירת מחדל מהקל לחמור) ונתוני "גזירת העונש" (כן/לא/הכל); להנחיות — בחירת מקורות. השדות של "גזירת העונש" נקראים מהגדרות עמוד גזרי הדין בסמים באתר z-g.co.il (`ZG_FILTER_CONFIG_URL`, מטמון שעה, רק בוליאנים על `meta.*` שקיימים בסכמת TAG-IT), ורשימת המקורות מ-`over-guidelines/facets`. הבחירה נשמרת ב-`conversations.search_setup` והשרת מחיל אותה על כל חיפוש בשיחה (כולל "תוצאות נוספות"), גם אם המודל שלח פרמטרים אחרים.
