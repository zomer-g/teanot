...
- **סרטון הדגמה במסך הכניסה** (`public/demo/`): הרצה אמיתית של המערכת בפרודקשן על כתב אישום בדוי בעבירות ירי (16.9.2026), שהוקלטה כזרם אירועים ונוגנה דרך ה-frontend האמיתי ב-Chrome ללא ממשק לצורך צילום. גזרי הדין, ההנחיות והסיכום הם הפלט שהמערכת החזירה בפועל. ללא קול ולכן עם תיאור מילולי מתחת לסרטון; מתנגן רק בלחיצה. דורש `media-src 'self'` ב-CSP.
